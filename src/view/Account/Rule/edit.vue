    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="110px">
      ...
      <el-form-item label="父节点" prop="pid">
        <el-select v-model="form.pid" placeholder="父节点" class="w-200" disabled>
          <el-option v-for="(item,index) in options" :label="item.title" :key="index" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      edit(form) {
        this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            this.apiPost('admin/adminrule/edit',this.form).then((res) => {
      ...
      getRules() {
        this.apiGet('admin/adminrule/index').then((res) => {
          this.handelResponse(res, (data) => {
      ...
      getRuleInfo() {
        ...
        this.apiGet('admin/adminrule/read?id=' + this.form.id).then((res) => {
          this.handelResponse(res, (data) => {
